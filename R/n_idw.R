...
n_idw <- function(roads, aprs, power =2){
   ...
   roads <- roads %>% mutate(centr = st_centroid(geometry)) %>% distinct(centr, .keep_all = TRUE) %>% select(-centr)
   ...
   e <- e %>%   mutate(xy = paste(.$X, .$Y)) %>% 
      group_by(xy) 
   e$EdgeID <- group_indices(e)
   e <- e %>% group_by(EdgeID) %>%  mutate(n = n()) %>% 
      filter(n>1) # n = 1 oznacza slepy koniec drogi wiec  sie nie nadaje na edge
   ...
   krzyz <- function(x){
      expand.grid(x$nodeID, x$nodeID) %>% filter(Var1 != Var2) %>% mutate(EdgeID = unique(x$EdgeID))
   }
   e2 <- e_list %>% purrr::map(\(x) krzyz(x)) %>% data.table::rbindlist() %>% 
      rename(all_of(c(from = "Var1", to = "Var2")))
   e1 <- filter(e, n==2) %>% select(nodeID, EdgeID) %>% 
   ...
   # tictoc::tic()
   e_dist <- e %>% rowwise() %>%  
      mutate(waga = sf::st_distance(filter(n1, nodeID==from), filter(n1, nodeID==to)))
   # tictoc::toc()
   
   library(tidygraph)
   # Tworzenie i poprawianie grafu
   graph <- tbl_graph(nodes = as_tibble(n), edges = as_tibble(e_dist), directed = FALSE)
   
   # IDW z pprow wg drog ----
   ppr_node <- graph %>% activate(nodes) %>% as_tibble() %>% filter(!is.na(r))
   new_n <- select(n, nodeID, r, geometry) %>% mutate(odl1=NA, odl2=NA, odl3=NA)
   # Na razie jako nowy graf
   graph1 <- tbl_graph(nodes = new_n, edges = as_tibble(e_dist), directed = FALSE)
   
   
   #tictoc::tic()
   for (i in 1:nrow(new_n)) { # Obliczenia odleglosci kazdej drogi od kazdego z 3 pprow
      cat("wiersz ", i, "z ", nrow(new_n), "\r" )
      
      graph1 <- graph1 %>% mutate(odl1 = ifelse(nodeID==new_n$nodeID[i],
                                                node_distance_from(ppr_node$nodeID[1], algorithm = "dijkstra", weights = waga),
                                                odl1),
                                  odl2 = ifelse(nodeID==new_n$nodeID[i],
                                                node_distance_from(ppr_node$nodeID[2], algorithm = "dijkstra", weights = waga),
                                                odl2),
                                  odl3 = ifelse(nodeID==new_n$nodeID[i],
                                                node_distance_from(ppr_node$nodeID[3], algorithm = "dijkstra", weights = waga),
                                                odl3),
      ) 
   }
   #tictoc::toc()
   ...
   # Obliczanie idw
   wynik <- graph1 %>% activate(nodes) %>% as_tibble() %>% 
      mutate(odl1 = 1/odl1^power, odl2 = 1/odl2^power, odl3 = 1/odl3^power) %>% # wagi - inverse distance
      ...
      mutate(odl1 = odl1*ppr_node$r[1], odl2 = odl2*ppr_node$r[2], odl3 = odl3*ppr_node$r[3]) %>% # wazone r
